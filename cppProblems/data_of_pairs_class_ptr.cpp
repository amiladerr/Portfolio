...
class StudentData {
  ...
  string name;
public:
  // IMPORTANT: default constructor is not necessary anymore!!!
...
class Collection {
  int capacity;
  StudentData **data;               // Dobule pointer (poiunter to pointer)
  int no_of_items;
public:
  Collection(int cap) {
    no_of_items = 0; capacity = cap;
    data = new StudentData*[cap];           // Array of pointers!!!
  }
  Collection(const Collection &c) {
    capacity = c.capacity; no_of_items = c.no_of_items;
    data = new StudentData*[c.capacity];
    for(int i = 0; i < no_of_items; i++)
      data[i] = new StudentData(*c.data[i]);      // this produces dynamic clone of *c.data[i]
  }
  ~Collection() {
    for(int i = 0; i < no_of_items; i++) delete data[i];
    delete[] data;
  }
...
void Collection::Insert(StudentData item) {
  ...
  data[no_of_items] = new StudentData(item);                   // Makes dynamic clone of item
  no_of_items++;
}
...
void Collection::ChangeNameByIndex(int index, string new_name) {
  for(int i = 0; i < no_of_items; i++)
    if(data[i]-GetIndex() == index) {
      *data[i] = StudentData(index, new_name;
      return;
    }
  throw "Not found!";
}
