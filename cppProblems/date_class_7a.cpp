...
//Date() { day = 1; month = 1; year = 2000; }                     // Alternative (longer, but faster) implementation
  Date(int day, int month, int year) { Set(day, month, year); }
  static bool IsLeapYear(int year);                               // static - these function do not know the object on which
  static int NoOfDays(int month, int year);                       //   the call is made ("this" does not exist in them)
...
int main() {
  ...
  cout << endl;
  cout << Date::NoOfDays(2, 2020) << endl;
  //Date::Display();
}
